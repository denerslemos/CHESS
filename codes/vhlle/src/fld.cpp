    ...
    for (int iy = 2; iy < ny - 2; iy++)
      for (int iz = 2; iz < nz - 2; iz++) {
        ...
        for (int isegm = 0; isegm < Nsegm; isegm++) {
          nelements++;
          ffreeze.precision(5);
          ffreeze << scientific << tau + cornelius->get_centroid_elem(isegm, 0)
                  << "  "
                  << scientific << getX(ix) + cornelius->get_centroid_elem(isegm, 1)
                  << "  "
                  << scientific << getY(iy) + cornelius->get_centroid_elem(isegm, 2);
                  if(nz > 6) {ffreeze << scientific << "  " << getZ(iz) + cornelius->get_centroid_elem(isegm, 3);}
                  else{ffreeze << scientific << "  " << getZ(iz);}
...
//          for (int ii = 0; ii < 4; ii++) ffreeze << setw(24) << dsigma[ii];
          for (int ii = 0; ii < 4; ii++) ffreeze << "  " << scientific << uC[ii];
//          ffreeze << setw(24) << TC << setw(24) << mubC << setw(24) << muqC
//                  << setw(24) << musC;
#ifdef OUTPI
          double picart[3];
//          /*pi00*/ picart[0] = ch * ch * piC[index44(0, 0)] +
...
//                                           sh * sh * piC[index44(3, 3)];
  ...
  //            sh * piC[index44(0, 1)] + ch * piC[index44(3, 1)];
          ...
          /*pi22*/ picart[2] = piC[index44(2, 2)];

  //        /*pi23*/ picart[index44(2, 3)] =
  //            sh * piC[index44(0, 2)] + ch * piC[index44(3, 2)];

  //        /*pi33*/ picart[3] = sh * sh * piC[index44(0, 0)] +
  //                                         ch * ch * piC[index44(3, 3)] +
  //                                         2. * sh * ch * piC[index44(0, 3)];
          for (int ii = 0; ii < 3; ii++) ffreeze << "  " << scientific << picart[ii];
          ffreeze << "  " << scientific << PiC << endl;
#else
    ...
    for (int iy = 2; iy < ny - 2; iy++)
      for (int iz = 2; iz < nz - 2; iz++) {
        ...
        if (isCorona && !isTail) {
          nelements++;
          ffreeze.precision(5);
          ffreeze << scientific << tau << "  " << scientific << getX(ix) + 0.5 * dx
                  << "  " << scientific << getY(iy) + 0.5 * dy << "  "
                  << scientific << getZ(iz) + 0.5 * dz;
          // ---- interpolation procedure
          double vxC = 0., vyC = 0., vzC = 0., TC = 0., mubC = 0., muqC = 0.,
                 musC = 0., piC[10], PiC = 0., nbC = 0.,
                 nqC = 0.;  // values at the centre, to be interpolated
