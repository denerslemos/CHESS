...
void write_text_file(const fs::path& output_dir, int width, int num,
  ...
  std::ostringstream padded_fname1{};
  padded_fname1 << std::setw(width) << std::setfill('0') << num << ".txt";
  fs::ofstream ofs1{output_dir / padded_fname1.str()};
  ...
  if (header) {
    ...
    ofs1 << std::setprecision(10)
        ...
        << "npart  " << event.npart()        << '\n';
    // Output ncoll if calculated
    ofs1 << "mult   " << event.multiplicity() << '\n';
    for (const auto& ecc : event.eccentricity())
      ofs1 << "e" << ecc.first << "    " << ecc.second << '\n';
  }
// ofs1 << "ncoll " << ncoll << '\n';
    ...
    if (hdf5::filename_is_hdf5(output_path)) {
      ...
      writers_.emplace_back(
        [output_path, width, header](int num, double impact_param,
          int ncoll, const Event& event) {
		if ((event.npart() >= event.npartmin() and event.npart() <= event.npartmax() and event.multiplicity() >= event.multmin() and event.multiplicity() <= event.multmax()) or (event.npart() >= event.npartmin() and event.npart() <= event.npartmax() and event.multmax() == 0. and event.multmin() == 0.) or (event.npartmax() == 0 and event.npartmin() == 0 and event.multmax() == 0. and event.multmin() == 0.) or (event.multiplicity() >= event.multmin() and event.multiplicity() <= event.multmax() and event.npartmax() == 0 and event.npartmin() == 0)){  
        write_text_file(output_path, width, num,
              impact_param, ncoll, event, header);}
//			  std::cout << event.npartmax() << "    " << event.npartmin()<< "    " << event.multmax()<< "    " <<event.multmin(); //Dener mudar aqui e ja eras
        }
      );
    }
  }
