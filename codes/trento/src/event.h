...
//Dener
  const int& npartmax() const
  { return npartmax_; }

  const int& npartmin() const
  { return npartmin_; }


  const double& multmax() const
  { return multmax_; }

  const double& multmin() const
  { return multmin_; }
  /// \rst
 ...
 private:
  ...
  int npart_;
  int npartmax_,npartmin_;  //Dener

  /// Multiplicity (total entropy).
  double multiplicity_;
  double multmax_,multmin_;  //Dener
