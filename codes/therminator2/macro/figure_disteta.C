...
// Create histograms
  ...
  H1D[2] = (TH1D*) H1D[0]->Clone("H2");  H1D[2]->SetTitle(", K^{#pm}");
  H1D[3] = (TH1D*) H1D[0]->Clone("H3");  H1D[3]->SetTitle("p, #bar{p}");
...
// Canvas
 ...
 ofstream wout;
  ...
  wout<< "#eta" << "		" << "h+-" << "		" << "pi+-"<< "		" << "K+-"<< "		" << "p+-" << endl;
  for (int i = 1;i< =XBins; i++){
  wout<<H1D[0]->GetBinCenter(i)<<"		"<<H1D[0]->GetBinContent(i)<<"		"<<H1D[0]->GetBinError(i)<<"		"<<H1D[1]->GetBinContent(i)<<"		"<<H1D[1]->GetBinError(i)<<"		"<<H1D[2]->GetBinContent(i)<<"		"<<H1D[2]->GetBinError(i)<<"		"<<H1D[3]->GetBinContent(i)<<"		"<<H1D[3]->GetBinError(i)<<endl;
