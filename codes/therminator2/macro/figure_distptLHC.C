...
#define  _FIGURE_NAME_ "fig_distptLHC"
#define  _N_HISTOGRAMS_ 3

void figure_distptLHC(TString aEventDir = "./events/", Int_t aEventFiles = 1)
{
// ##########################################################################
// # READ ROOT FILES
// ##########################################################################
...
// Create histograms
  ...
  H1D[0]->GetYaxis()->SetTitle("dN/(2 #pi p_{T} dp_{T} d#eta)");
  H1D[0]->Sumw2();
...
// Fill histograms
  ...
  for(Int_t i=0; i<Chain->GetEntries(); i++) {
    ...
    if(Particle.e == Particle.pz)
      continue;
    Rap = 0.5 * TMath::Log((P+Particle.pz) / (P-Particle.pz));
...
// Canvas
  ...
  for (int i = 1;i< =XBins; i++){
    wout<<H1D[0]->GetBinCenter(i)<<" "<<H1D[0]->GetBinContent(i)/2.<<" "<<H1D[0]->GetBinError(i)<<endl;
