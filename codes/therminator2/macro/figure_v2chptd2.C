...
#define  _FIGURE_NAME_ "fig_v2chpt2"
#define  _N_HISTOGRAMS_ 3

void figure_v2chptd2(TString aEventDir = "./events/", Int_t aEventFiles = 1)
{
// ##########################################################################
// # READ ROOT FILES
// ##########################################################################
...
// Fill histograms
  ...
  Float_t angleA, angleB, a2, b2, c2, d2, aa2=0., bb2=0., cc2=0., dd2=0., RR;
  Int_t   pid;
  for(Int_t i=0; i<Chain->GetEntries(); i++) {
    ...
    if(Pt == 0.0)
      continue;
//    if( Rap > 0.0 && Rap < 1.0 ){
//	fangA=atan2(Particle.py,Particle.px);
//}
//    if( Rap < 0.0 && Rap > -1.0 ){
//	fangB=atan2(Particle.py,Particle.px);
//}
...
if ( Rap < 0.0  and Rap > -(0.075) ){
    ...
    angleA=atan2(bb2,aa2)/2.;
    angleB=atan2(dd2,cc2)/2.;
    cout<<angle*180./3.14159265<<endl;
    RR=TMath::Sqrt(cos(2*(angleA-angleB)));
    cout << RR << endl;
  for(Int_t i=0; i<Chain->GetEntries(); i++) {
    Chain->GetEntry(i);
    if(Particle.e == Particle.pz)
