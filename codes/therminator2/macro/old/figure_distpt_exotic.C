...
#define  _FIGURE_NAME_ "fig_distpt_exotic"
#define  _N_HISTOGRAMS_ 5

void figure_distpt_exotic(TString aEventDir = "./events/", Int_t aEventFiles = 1)
{
// ##########################################################################
// # READ ROOT FILES
// ##########################################################################
...
// Create histograms
  H1D[0] = new TH1D("H0", "#rho", XBins, XMin, XMax);
  H1D[0]->GetXaxis()->SetTitle("p_{T} [GeV/c]");
  H1D[0]->GetYaxis()->SetTitle("dN/(2 #pi p_{T} dp_{T} dy)");
  H1D[0]->Sumw2();
  H1D[1] = (TH1D*) H1D[0]->Clone("H1");  H1D[1]->SetTitle("K^{0*}");
  H1D[2] = (TH1D*) H1D[0]->Clone("H2");  H1D[2]->SetTitle("#phi");
  H1D[3] = (TH1D*) H1D[0]->Clone("H3");  H1D[3]->SetTitle("#Lambda^{0}");
  H1D[4] = (TH1D*) H1D[0]->Clone("H4");  H1D[4]->SetTitle("#Omega^{-}");
// Fill histograms
  Float_t Rap, Pt;
  Int_t   pid;
...
// Canvas
  ...
  gPad->SetLogy();
// Histogram legend
  TLegend* LegendPart	= new TLegend(0.53, 0.69, 0.69, 0.92, "",	"brNDC");
...
// Plot
  H1D[0]->SetMinimum(0.001);
  H1D[0]->SetTitle("p_{T} distribution (exotic)");
  H1D[0]->Draw();
  H1D[1]->Draw("SAME");
  H1D[2]->Draw("SAME");
  H1D[3]->Draw("SAME");
