...
#define  _FIGURE_NAME_ "fig_distpt2"
#define  _N_HISTOGRAMS_ 3

void figure_distpt2(TString aEventDir = "./events/", Int_t aEventFiles = 1)
{
// ##########################################################################
// # READ ROOT FILES
// ##########################################################################
...
// Canvas
 ...
 //   cout<<H1D[1]->GetBinCenter(i)<<" "<<((H1D[1]->GetBinContent(i)+H1D[2]->GetBinContent(i))/2.)<<endl;
 //   wout<<H1D[1]->GetBinCenter(i)<<" "<<((H1D[1]->GetBinContent(i)+H1D[2]->GetBinContent(i))/2.)<<" "<<((H1D[1]->GetBinError(i)+H1D[2]->GetBinError(i))/2.)<<endl;
