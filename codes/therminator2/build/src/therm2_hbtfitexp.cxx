...
// ############################################################## 
  ...
  tOutTextName = sEventDir + "hbtradii.dat";
...
Double_t fungek(Double_t *x, Double_t *par)
{
  Double_t qosq = TMath::Abs(x[0]);
  Double_t qssq = TMath::Abs(x[1]);
  Double_t qlsq = TMath::Abs(x[2]);

  Double_t paq;

  Double_t lam =  TMath::Abs(par[1]);

  Double_t gpart = exp((-par[2]*qosq-par[3]*qssq-par[4]*qlsq)/0.197326968);
